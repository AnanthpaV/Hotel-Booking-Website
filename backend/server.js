...
app.get('/api/search', async (req, res) => {
    ...
    try {
        // Prepare a search regex from the query (case-insensitive)
        const searchRegex = new RegExp(query, 'i');

        // Fetch matching places based on title, address, or hotel name
        ...
        }).select('title address photos price rating hotelName');

        // Extract the IDs of the matching places
        const matchingIds = matchingPlaces.map(place => place._id);

        // Fetch the remaining places excluding matching ones, sorted alphabetically by address
        const remainingPlaces = await Place.find({
            _id: { $nin: matchingIds }
        })
        .sort({ address: 1 })  // Alphabetical sorting
        .select('title address photos price rating hotelName');

        // Function to format places into consistent structure
        ...
        const formattedResults = {
            matching: matchingPlaces.map(formatPlace),
            remaining: remainingPlaces.map(formatPlace)
        };
...
app.get('/api/nearby', async (req, res) => {
    ...
    try {
        // Extract only the city or main part from the address
...
app.post('/api/experiences', async (req, res) => {
    try {
        const { title, description, location, price, image, host, date, duration, maxGroupSize, category } = req.body;
...
// Add this new route handler for a single experience
